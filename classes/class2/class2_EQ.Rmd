...
For loop: Repetition (forever or as long as you want)
While loop: You don't know how many times you want to repeat but until a certain event
...
clothing_advice <- function(temperature) {
  if (temperature > 25) { print("What about wearing a t-shirt?") }
  else if (temperature <= 25 && temperature >= 15) { print("You should maybe wear a light jacket") }
  else { print("A warm coat may be the right decision") }
...
### Exercise 0 and 5.5 are the most relevant ones for the portfolio
...
# Simulate 400 births by using the binomial distribution
n_girls <- rbinom(1, 400, 0.488)
print(n_girls) # Shows what could happen in 400 births

# Now with 100 births and making a vector n_girls to store the simulations
n_sims <- 1000
n_girls <- rep(NA, n_sims)
for (s in 1:n_sims) {
  n_girls[s] <- rbinom(1, 400, 0.488)
} # You can also use the old method
hist(n_girls) # Making a histogram

# Accounting for twins
n_sims <- 1000
n_girls <- rep(NA, n_sims) # number of repetitions


for (s in 1:n_sims) {
 birth_type <- sample(c("fraternal twin", "identical twin", "single birth"),
                     size = 400, replace = TRUE, prob = c(1/125, 1/300, 1 - 1/125 - 1/300)) #Make a list of birthtypes and probability 
}

girls <- rep(NA, 400)

for (i in 1:400) { # vector length is 400
  if (birth_type[i]=="single birth"){
    girls[i] <- rbinom(1, 1, 0.488) # one girl
  } else if (birth_type[i]=="identical twin") {
    girls[i] <- 2*rbinom(1, 1, 0.495)
  } else if (birth_type[i]=="fraternal twin") {
    girls[i] <- rbinom(1, 2, 0.495) # one girl and 1 boy
  }
} #It can also be performed without loop
n_girls <- sum(girls)
...
#Making a list to get the numbers - ChatGPT
...
mean(results) #Getting the mean
sd(results) # Getting the standard deviation

hist(results) #Simple plot
...
# Simulate differences - ChatGPT
simulated_differences <- replicate(num_sim, mean(rnorm(100, mu_men, sigma_men)) - mean(rnorm(100, mu_women, sigma_women)))

# Plot histogram - ChatGPT
hist(simulated_differences, main="Distribution of x - y", xlab="x - y", col="lightblue", border="black")
