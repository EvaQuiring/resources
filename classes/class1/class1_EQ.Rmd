...
To simulate data means to make up your own data which doesn't relate to the real world.
...
There are 20 datapoint with a mean of 1.5 and a standard deviation of 10. This should be somewhat normal distributed.
...
```
We get different results from what we set. This is caused by the random function rnorm(), and the values of the mean and the standard deviation are different because there is a limited amount of data points to let them fit in the normal distribution. 
...
If we want to predict the future - e.g. when we want to see how something changes which could be the Covid-19 pandemic where we wanted to see how many people get sick if the politicians decided something.

#### (b) Exploring associations
- Is there an association between mood and ice cream flavor or weather and if people go voting?

#### (c) Extrapolation
- Sentiment score --> how we talk about the news and how they affect us

#### (d) Causal inference
- Does mood depend on ice cream flavor
...
- It's a measure that is precise but not measures what you want. E.g. peoples body temperature to say something about their mood when they eat ice cream
...
# define the values that we know from his description 🙂
mu <- 0.0
sigma <- 1.0
tau <- 1.0
